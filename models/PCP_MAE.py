...
class Attention(nn.Module):
    ...
    def forward(self, x, y=None):    # y as q, x as q, k, v
        ...
        B, N, C = x.shape
        L = y.shape[1]
        x = torch.cat([x, y], dim=1)    # NOTE cat
...
class PCP_MAE(nn.Module):
    ...
    def forward(self, pts, vis = False, **kwargs):
        ...
        if self.ablation == 'MAE_pred_pos_not_to_decoder' or self.ablation == 'pred_pos_only' or self.ablation == 'reconstruction_only':
            pos_full = torch.cat([pos_emd_vis, 
                                  self.MAE_encoder.pos_embed(get_pos_embed(self.trans_dim, center[mask].reshape(B, -1, 3)))], dim=1)
        else:
        ...
        # < 0 symbolize the loss that doesn't use
        if self.ablation == 'pred_pos_only_to_decoder' or self.ablation == 'pred_pos_only': # pred_pos_only means not to decoder
            return -loss1, loss2    
        elif self.ablation == 'MAE_not_pred_pos_but_to_decoder' or self.ablation == 'reconstruction_only':
            return loss1, -self.config.ita * loss2
        elif self.ablation == 'MAE_pred_pos_not_to_decoder':
            return loss1, self.config.ita * loss2 
