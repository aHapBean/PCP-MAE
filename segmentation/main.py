...
def parse_args():
    ...
    parser.add_argument('--model-prefix', type=str, default=None, help='ours: MAE_encoder(TODO change), Point-MAE: MAE_encoder, PointGPT: Banned in this version')
...
def main(args):
    ...
    # print("here") # 这里越了两级，所以当 e.g. 888-epoch/0的第一个888-epoch不存在时会报错
    # NOTE 不允许重新建dir
    exp_dir.mkdir(exist_ok=False)    # 如果exist_ok设置为True，并且目录已经存在，那么不会引发异常，mkdir()方法什么也不会做。
    # 如果exist_ok设置为False（默认值），并且目录已经存在，那么会引发FileExistsError异常。
    ...
    def add_weight_decay(model, weight_decay=1e-5, skip_list=()):
        if args.learning_rate in [0.0001, 0.00005]:    # recon 配置
            decay = []
            no_decay = []
            finetune_head = []
        ...
        elif args.learning_rate == 0.0002 or args.learning_rate == 0.0003 or args.learning_rate == 0.0005 or args.learning_rate == 0.001:  # 正常配置
            decay = []
            no_decay = []
            for name, param in model.named_parameters():
                if not param.requires_grad:
