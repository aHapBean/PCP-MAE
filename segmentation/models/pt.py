...
class get_model(nn.Module):
    def __init__(self, cls_dim):
        ...
        self.convs1 = nn.Conv1d(3392, 512, 1)   # 3776 -> 3392
    ...
    def load_model_from_ckpt(self, bert_ckpt_path, model_prefix):
        # prefix: ours: MAE_encoder
        # PointMAQ: MAE_encoder
        # PointGPT: GPT_transformer
        assert model_prefix in ['PQAE_encoder', 'MAE_encoder']
    ...
    def forward(self, pts, cls_label):
        ...
        feature_list = self.blocks(x, pos) # 16,128,384 || 16, 3, 384
        # print("feature {}, feature[0] {}, cls {}".format(len(feature_list), feature_list[0].shape, cls.shape))
        # feature 3, feature[0] torch.Size([16, 128, 384]), cls torch.Size([16, 3, 384])
        # cls = cls.max(1)[0]
        # print(cls.shape)
        # print("cls shape {}".format(cls.shape)) # cls shape torch.Size([16, 384])
        ...
        # cls = cls.unsqueeze(2).repeat(1, 1, N)
        # print("cls sahpe: ", cls.shape) # cls sahpe:  torch.Size([16, 384, 2048])
