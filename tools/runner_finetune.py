...
def run_net(args, config, train_writer=None, val_writer=None):
    ...
    if hasattr(config.model, 'no_aug') and config.model.no_aug:
        train_transforms = transforms.Compose([])
    elif config.dataset.train._base_.NAME == "ModelNet":
        train_transforms = transforms.Compose([
            data_transforms.PointcloudScaleAndTranslate(),
        ])
    elif 'downstream_translate_scale' in args.exp_name or 'downstream_scale_translate' in args.exp_name:
        train_transforms = transforms.Compose([
            data_transforms.PointcloudScaleAndTranslate(),
        ])
    else:
    ...
    for epoch in range(start_epoch, config.max_epoch + 1):
        ...
        if epoch % args.val_freq == 0 and epoch != 0:
            ...
            metrics = validate(base_model, test_dataloader, epoch, val_writer, args, config, best_metrics, logger=logger)     # NOTE validate就是测试
