...
def run_net(args, config, train_writer=None, val_writer=None):
    ...
    if config.model.aug == 'shear':
        train_transforms = transforms.Compose(
            [
                # data_transforms.PointcloudScale(),
                data_transforms.PointcloudScaleAndTranslate(),
                data_transforms.PointcloudRotate(),
                data_transforms.PointcloudShear(),
        ...
        print("Add Point cloud shear to pretrain !!!")
    elif config.model.aug == 'rotate':
        train_transforms = transforms.Compose(
            [
                data_transforms.PointcloudRotate(),
            ]
        )
    elif config.model.aug == 'scale_translate':
        train_transforms = transforms.Compose(
            [
                data_transforms.PointcloudScaleAndTranslate(),
            ]
        )
    elif config.model.aug == 'rotate_scale_translate':
        train_transforms = transforms.Compose(
            [
                data_transforms.PointcloudRotate(),
                data_transforms.PointcloudScaleAndTranslate(),
        ...
        # raise ValueError
    elif config.model.aug == 'horizontal_flip':
        train_transforms = transforms.Compose(
            [
                data_transforms.RandomHorizontalFlip(),
            ]
        )
    elif config.model.aug == 'no_aug':
        train_transforms = transforms.Compose(
            [
                # data_transforms.RandomHorizontalFlip(),
            ]
        )
    elif config.model.aug == 'None' or config.model.aug == 'scale_translate_rotate':
        # by default
        train_transforms = transforms.Compose(
            [
                # data_transforms.PointcloudScale(),
    ...
    # trainval
    # training
    # calculate the GFLOPS and the parameters
    ...
    for epoch in range(start_epoch, config.max_epoch + 1):
        ...
        losses = AverageMeter(['Loss', 'loss2'])
        ...
        for idx, (taxonomy_ids, model_ids, data) in enumerate(train_dataloader):
            ...
            assert not (loss1 < 0 and loss2 < 0)
            if loss1 < 0:
                loss = loss2
            elif loss2 < 0:
                loss = loss1
            ...
            if args.distributed:
                ...
                if loss1.item() < 0:
                    losses.update([-loss1.item() * 1000, loss2.item()*1000])
                elif loss2.item() < 0:
                    losses.update([loss1.item()*1000, -loss2.item()*1000])
                else:
                    losses.update([loss1.item()*1000, loss2.item()*1000])
